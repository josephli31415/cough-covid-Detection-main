...
def generate_report(user_info, prediction, features):
    ...
    p.drawString(100, 750, "Tuberculosis Detection Report")
    ...
    p.drawString(100, 670, f"Prediction: {'Tuberculosis' if prediction[0] >= 0.9 else 'Non-Tuberculosis'}")
...
<script>
  document.addEventListener('DOMContentLoaded', function() {
    ...
    tour.addStep({
      id: 'welcome',
      text: 'Welcome to the Tuberculosis Detection App! This tour will guide you through the steps to use this app.',
...
if uploaded_file is not None:
    ...
    if st.button('Submit'):
        ...
        if prediction[0] >= 0.87:
            st.success("Prediction: Tuberculosis")
        else:
            st.success("Prediction: NonTuberculosis")
        ...
        st.download_button(label="Download Report", data=buffer, file_name="TB_Report.pdf", mime="application/pdf")
